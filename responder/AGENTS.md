...
> This file is for **Codex CLI and other agents following the `AGENTS.md` convention.**
> The equivalent instructions for Claude Code are in [`CLAUDE.md`](CLAUDE.md) (nearly identical content; only the runtime assumptions differ).
...
Do **not** bake knowledge about a specific use case (chat / agent orchestration / RAG / batch processing, etc.) into these instructions. Decide on the spot from the request contents. Assume the same proxy may later be used for other purposes (another agent / raw chat / a puppetllm OSS demo, etc.).

---

## Codex-specific notes (important)

1. **Network permission**: Codex CLI sandboxes commands by default and **blocks network access**. Even for a localhost proxy, you must run in a network-enabled sandbox to let `curl` through (equivalent of `--full-auto` / an approval setting / `config.toml` network allowance). It is all the more required for another host (when `$PUPPET_URL` is remote). **First confirm that `curl $PUPPET_URL/_control/health` works** before entering the loop.
2. **Command execution timeout**: keep the long-poll `N` (`wait_for_pending?timeout=N`) **well below your runtime's per-command execution timeout** (if a command is killed mid-flight you drop the pending). If unsure, start conservatively at around **`timeout=90`–`120`** and increase once stable. The server-side cap is 600.
3. **Approval mode**: respond/auto are POSTs with side effects (mutating proxy state). If not auto-approved, allow these commands while running the responder so the loop is not halted by an approval prompt on each `curl`.
...
```
1. curl GET $PUPPET_URL/_control/wait_for_pending?timeout=120   (long-poll)
...
Keep `?timeout=N` below your runtime's per-command execution timeout (see "Codex-specific notes" #2 above).
...
> The server returns `400 {"error":"invalid JSON body"}` for a broken body. **Use a file or a heredoc:**
>
> ```bash
...
- Do **not** write meta self-references into the response, e.g. "I am the puppet / responder / Codex (the answerer)"
...
- Generating responses consumes **this responder agent's context**
- Returning the essence in 10 lines instead of 100 fits the responsibility better and also saves context
...
- `CLAUDE.md` — equivalent instructions for Claude Code (differs only in runtime assumptions)

Specs that depend on the caller's context (which project uses the proxy) — agent roles, markers, etc. — should be in that request's system prompt. Do not write them in this AGENTS.md (keep it neutral).
